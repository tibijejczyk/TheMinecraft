...
public class PowerTools
{
	...
	public void preInit(FMLPreInitializationEvent event)
	{
		if (event.getSide() == Side.SERVER)
		{
			...
			if (cfgHighestDimension < 1)
				cfgHighestDimension = 1; // Must be at least 1 to include 'The
											// End'.
			...
			for (int i = 2; i <= cfgHighestDimension; i++)
			{
				...
				proMapLimitWorld.comment = "Setting this to true will stop players from moving beyond the specified coordinates on the map in "
						+ worldNumber;
			...
			if (cfgMapLimitNether.enabled)
				cfgAnyWorldMapLimit = true;
			for (int i = 0; i <= cfgHighestDimension; i++)
			{
				if (cfgMapLimitDimemsion[i].enabled)
					cfgAnyWorldMapLimit = true;
			}
		}
	}
	...
	public static int getNumberForPlayerName(String plName)
	{
		...
		{
			if (plInfo[i].name.equals(plName))
				nr = i;
		}
		return nr;
	}
	...
	public static void addNameToChatList(String playerName, String chatName)
	{
		int nr = getNumberForPlayerName(playerName);
		if (nr > -1)
			plInfo[nr].chatToPlayers += chatName + "|";
	}
	...
	public static String encrypt(String stringIn, String codeIn)
	{
		...
		for (i = 0; i < codeIn.length(); i++)
		{
			...
			codeLetter = 1;
			if (oneLetter > 47 && oneLetter < 58)
				codeLetter = (char) (oneLetter - 47);
			if (oneLetter == 95)
				codeLetter = 11;
			if (oneLetter > 96 && oneLetter < 123)
				codeLetter = (char) (oneLetter - 85);
			newCode += codeLetter;
		}
	...
	public static boolean checkForValidCharacters(String inString)
	{
		...
		for (i = 0; i < inString.length(); i++)
		{
			...
			if (oneLetter < 48)
				valid = false;
			if (oneLetter > 57 && oneLetter < 65)
				valid = false;
			if (oneLetter > 90 && oneLetter < 95)
				valid = false;
			if (oneLetter == 96)
				valid = false;
			if (oneLetter > 122)
	...
	public static boolean stringlistContains(String stringList, String checkString)
	{
		...
		for (int i = 0; i < stringList.length(); i++)
		{
			...
			if (oneLetter.equals(","))
			{
				if (tmpString.equals(checkString))
					check = true;
				tmpString = "";
			} else
			{
